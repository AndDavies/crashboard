...
// GitHub token handling - safely get from environment (server-side only)
...
export async function POST(request: Request) {
  try {
    ...
    const serializedStructure = {
      files: repoStructure.files,
      routes: Object.fromEntries(repoStructure.routes),
      apis: Object.fromEntries(repoStructure.apis)
    };
    
    return NextResponse.json({ 
      data: serializedStructure 
    });
  } catch (error) {
    const errorMessage = (error as Error).message || 'Failed to analyze repository';
 ...
 */
async function fetchRepositoryStructure(owner: string, repo: string): Promise<RepoStructure> {
  try {
